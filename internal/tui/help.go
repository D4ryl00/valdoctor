...
func helpContent(m Model) string {
	...
	row("n / p", "go to the next newer / older retained height")
	row("N", "jump to the latest retained height and keep following new blocks")
	row("P", "jump to the oldest retained height in memory")
	...
	nl()
	row("Validator vote grid", "one row per validator slot showing that validator's observed vote")
	...
	nl()
	note("This grid is NOT the minimal subset of votes that first formed +2/3.")
	note("It shows per-validator observed votes for the round.")
	...
	row("Precommit narrative", "summarises what 2/3+ precommitted; this is the consensus outcome view")
	nl()
	...
	nl()
	row("ok Xs", "vote received Xs after it was cast, before the receiver first logged +2/3")
	row("late Xs", "vote received on or after the receiver first logged +2/3 for that vote set")
	row("after+2/3", "receiver had already reached +2/3, but only aggregate/quorum-level evidence exists for this vote on that receiver")
	...
	note("'late' in propagation means the vote was seen on or after that receiver had")
	note("already crossed +2/3 for the same vote set; it did not help that receiver form quorum.")
	nl()
